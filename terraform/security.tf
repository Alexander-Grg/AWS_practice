...
resource "aws_security_group" "default_sg" {
  ...
  ingress {
    ...
    to_port         = 5432
    protocol        = "tcp"
    security_groups = [aws_security_group.crud_srv_sg.id]
  }
...
# Security Group 3: crud-srv-sg
resource "aws_security_group" "crud_srv_sg" {
  name        = "crud-srv-sg"
...
output "crud_srv_sg_id" {
  description = "ID of the crud-srv-sg security group"
  value       = aws_security_group.crud_srv_sg.id
}

# IAM Role for cruddur-post-confirmation Lambda Function
resource "aws_iam_role" "cruddur_post_confirmation_role" {
  name = "cruddur-post-confirmation-role"
...
resource "aws_iam_role_policy_attachment" "cruddur_post_confirmation_vpc_policy" {
...
resource "aws_iam_policy" "lambda_ec2_policy" {
  name        = "${var.function_name_cruddur_messaging_stream}-ec2-policy"
...
resource "aws_iam_role" "messaging_stream_lambda_role" {
  name = "${var.function_name_cruddur_messaging_stream}-role"
...
resource "aws_iam_policy" "lambda_dynamodb_policy" {
  name        = "${var.function_name_cruddur_messaging_stream}-dynamodb-policy"
...
resource "aws_cognito_user_pool" "main" {
  ...
  lambda_config {
    post_confirmation = aws_lambda_function.cruddur_post_confirmation.arn
  }
}

resource "aws_cognito_user_pool_client" "client" {
  name = "cruddur-app-client"
...
resource "aws_cognito_identity_pool" "main" {
  identity_pool_name               = "cruddur-identity-pool"
...
resource "aws_iam_role" "authenticated" {
  name = "cruddur-cognito-authenticated-role"
...
resource "aws_iam_role_policy" "authenticated_policy" {
  name = "cruddur-authenticated-policy"
