...
locals {
  ...
  db_connection_string = "postgresql://${aws_db_instance.cruddur_rds_instance.username}:${aws_db_instance.cruddur_rds_instance.password}@${aws_db_instance.cruddur_rds_instance.endpoint}/${aws_db_instance.cruddur_rds_instance.db_name}"
}
...
resource "local_file" "cruddur_messaging_stream_source" {
...
data "archive_file" "cruddur_messaging_stream_lambda_zip" {
  type        = "zip"
  source_file = local_file.cruddur_messaging_stream_source.filename
...
data "archive_file" "psycopg2_layer_zip" {
  ...
  excludes    = local.is_codespaces ? ["requirements.txt"] : []
  
  depends_on = local.is_codespaces ? [null_resource.build_psycopg2_layer[0]] : []
}
...
resource "aws_iam_role_policy_attachment" "lambda_ec2_policy_attachment" {
  role       = aws_iam_role.cruddur_post_confirmation_role.name
...
resource "aws_lambda_function" "cruddur_post_confirmation" {
  ...
  role             = aws_iam_role.cruddur_post_confirmation_role.arn
...
resource "aws_lambda_permission" "allow_cognito" {
  ...
  function_name = aws_lambda_function.cruddur_post_confirmation.function_name
  principal     = "cognito-idp.amazonaws.com"
}

# CloudWatch Log Group for Messaging Stream Lambda
resource "aws_cloudwatch_log_group" "cruddur_messaging_stream_logs" {
  name              = "/aws/lambda/${var.function_name_cruddur_messaging_stream}"
  retention_in_days = 14

  tags = {
    Name        = "${var.function_name_cruddur_messaging_stream}-logs"
    Environment = var.environment
  }
}

# Messaging Stream Lambda Function
resource "aws_lambda_function" "cruddur_messaging_stream" {
  filename         = data.archive_file.cruddur_messaging_stream_lambda_zip.output_path
  function_name    = var.function_name_cruddur_messaging_stream
  ...
  logging_config {
    log_format = "Text"
    log_group  = aws_cloudwatch_log_group.cruddur_messaging_stream_logs.name
  }
  ...
  depends_on = [
    ...
    aws_cloudwatch_log_group.cruddur_messaging_stream_logs,
    data.archive_file.cruddur_messaging_stream_lambda_zip
  ]

  tags = {
    Name        = var.function_name_cruddur_messaging_stream
    Environment = var.environment
    EnvironmentType = local.is_codespaces ? "codespaces" : "local"
  }
}
...
resource "aws_lambda_event_source_mapping" "messaging_stream_trigger" {
  event_source_arn  = aws_dynamodb_table.cruddur_messages.stream_arn
  function_name     = aws_lambda_function.cruddur_messaging_stream.arn
...
output "cruddur_post_confirmation_lambda_function_arn" {
  description = "ARN of the Lambda function"
  value       = aws_lambda_function.cruddur_post_confirmation.arn
}

output "post_confirmation_lambda_function_name" {
  description = "Name of the Lambda function"
  value       = aws_lambda_function.cruddur_post_confirmation.function_name
}

output "post_confirmation_lambda_role_arn" {
  description = "ARN of the Lambda IAM role"
  value       = aws_iam_role.cruddur_post_confirmation_role.arn
}
...
output "cruddur_messaging_stream_lambda_function_arn" {
  description = "ARN of the Lambda function"
  value       = aws_lambda_function.cruddur_messaging_stream.arn
}

output "messaging_stream_lambda_function_name" {
  description = "Name of the Lambda function"
  value       = aws_lambda_function.cruddur_messaging_stream.function_name
}
...
output "messaging_stream_lambda_log_group_name" {
  description = "Name of the CloudWatch log group"
  value       = aws_cloudwatch_log_group.cruddur_messaging_stream_logs.name
}