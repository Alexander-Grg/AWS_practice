...
resource "aws_cloudwatch_log_group" "cruddur_fargate_cluster_managed" {
...
resource "aws_cloudwatch_metric_alarm" "rds_cpu_high" {
  ...
  dimensions = {
    DBInstanceIdentifier = aws_db_instance.cruddur_rds_instance.identifier
  }
...
# DynamoDB CloudWatch Alarms (based on your cruddur-messages table)
resource "aws_cloudwatch_metric_alarm" "dynamodb_read_throttled_events" {
  ...
  dimensions = {
    TableName = aws_dynamodb_table.cruddur_messages.name
  }
    ...
    widgets = [
      {
        ...
        properties = {
          metrics = [
            ["AWS/RDS", "CPUUtilization", "DBInstanceIdentifier", aws_db_instance.cruddur_rds_instance.identifier],
            [".", "DatabaseConnections", ".", "."],
            [".", "FreeStorageSpace", ".", "."]
          ]
          view    = "timeSeries"
      ...
      },
      {
        ...
        properties = {
          metrics = [
            ["AWS/DynamoDB", "ConsumedReadCapacityUnits", "TableName", aws_dynamodb_table.cruddur_messages.name],
...
resource "aws_cloudwatch_log_metric_filter" "error_count" {
  ...
  log_group_name = aws_cloudwatch_log_group.cruddur_fargate_cluster_managed.name
  pattern        = "ERROR"
