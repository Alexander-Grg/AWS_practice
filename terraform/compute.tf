...
resource "aws_lambda_function" "cruddur_post_confirmation" {
  ...
  environment {
    variables = {
      PROD_CONNECTION_STRING = "postgresql://${aws_db_instance.cruddur_rds_instance.username}:${aws_db_instance.cruddur_rds_instance.password}@${aws_db_instance.cruddur_rds_instance.endpoint}/${aws_db_instance.cruddur_rds_instance.db_name}"
    }
  }
